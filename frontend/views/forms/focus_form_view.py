...
def create_focus_form_view(page: ft.Page):
    try:
        ...
        # =====================================================================
        # 1. TÉCNICA DO IMORTAL (SINGLETON)
        # =====================================================================
        # Isso garante que o GPS viva na memória (page) e não na tela (view)
        # Evita o erro de "Unknown control" (Tela Vermelha)
        ...
        # Atualizamos o "cérebro" do picker para responder a ESTA tela
        ...
        # =====================================================================
        # 2. LÓGICA E FUNÇÕES (DEFINIDAS ANTES DA UI)
        # =====================================================================
        ...
        # --- MODAL DE SELEÇÃO MANUAL DE RUA ---
        ...
        # --- FUNÇÕES DE BUSCA (CEP/RUA) ---
        # Definidas aqui para o Python ler antes de criar os campos
        def search_cep(e):
        ...
        # Funções que conectam o GPS Global a esta tela
        ...
        # Vincula GPS Imortal às funções desta tela (Cérebro novo, corpo velho)
        ...
        def get_gps_click(e):
            ...
            # 15s para o satélite responder
            threading.Timer(15.0, gps_timeout_handler).start()
            try:
                # Chama o Singleton
                page.vigi_geolocator.get_current_position(accuracy=ft.GeolocatorPositionAccuracy.HIGH)
            except Exception as ex:
                print(f"Erro GPS Call: {ex}")
                try_ip_location()

        # =====================================================================
        # 3. CRIAÇÃO DOS CAMPOS VISUAIS (UI)
        # =====================================================================
        # Como todas as funções (search_cep, etc) já foram lidas acima, isso aqui
        # vai funcionar liso.
        ...
        # Botão GPS V13
        btn_gps = ft.ElevatedButton("GPS V13 BLINDADO", icon=ft.Icons.LOCATION_ON, bgcolor="#39BFEF", color="white", width=float("inf"), style=ft.ButtonStyle(shape=ft.RoundedRectangleBorder(radius=8)), on_click=get_gps_click)
        ...
        # Hooks para atualizar labels do modal GPS
        ...
        def close_gps_modal():
            gps_overlay.visible = False; btn_gps.text = "GPS V13 BLINDADO"; btn_gps.icon = ft.Icons.LOCATION_ON; btn_gps.disabled = False; page.update()
            ...
            add_btn_row = ft.Row(
                controls=[
                    ft.Container(
                        ...
                        on_click=lambda _: page.vigi_file_picker.pick_files(allow_multiple=True, file_type=ft.FilePickerFileType.IMAGE)
                    ),
